...
import { Plus, MapPin, Thermometer, ChevronDown, ChevronUp, Loader2, PackageCheck, AlertTriangle } from "lucide-react";
...
export default function DistributorRequests() {
  ...
  const confirmReceive = async (e) => {
    ...
    try {
      ...
      if (res.mismatch?.mismatch) {
        push(`Received with a discrepancy: expected ${receiveTarget.qtyDispatched}, got ${qtyReceivedInput}. Flagged as an anomaly for Admin review.`, "error");
      } else {
        push("Shipment received in full. Distributor inventory updated.", "success");
      }
    ...
    <div className="space-y-4">
                  ...
                  <td className="space-x-1.5">
                    {r.status === "dispatched" && (
                      <>
                        ...
                        <button onClick={() => openReceive(r)} className="btn bg-distributor text-white hover:bg-distributor/90 !py-1 !px-2.5 text-xs">
                    ...
                    <td colSpan={8} className="bg-slate-50 px-6 py-4">
                      <div className="grid md:grid-cols-3 gap-4 text-xs">
                        <div>
                          ...
                          {r.qtyDispatched != null && r.qtyReceived != null && r.qtyDispatched !== r.qtyReceived && (
                            <p className="text-red-600 font-semibold flex items-center gap-1 mt-1"><AlertTriangle size={12} /> Quantity mismatch flagged</p>
                          )}
                        </div>
                        <div>
                          ...
                          {r.coldChainLog.length === 0 ? <p className="text-slate-400">Not applicable or no readings.</p> : (
                            <ul className="space-y-1">
                              {r.coldChainLog.map((c, i) => (
                                <li key={i} className={`font-mono ${c.alert ? "text-red-600 font-semibold" : "text-slate-500"}`}>{c.temp}&deg;C / {c.humidity}% {c.alert ? "⚠" : ""}</li>
                              ))}
                            </ul>
                          )}
                        </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Request Stock from Vendor">
        <form onSubmit={raiseRequest} className="space-y-3">
          ...
          <label className="text-xs font-medium text-slate-600 space-y-1 block">
            Priority
            <select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })} className="input focus:ring-distributor/20 focus:border-distributor">
