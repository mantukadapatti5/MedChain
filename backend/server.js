...
// Level 1 notification bridge: successful domain events become persistent
// notifications. eventKey prevents repeated dashboard refreshes from spamming.
app.use((req, res, next) => {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300 && req.user) {
        ...
        if (path === "/api/admin/shortage-risk" && Array.isArray(body)) {
          body.filter((row) => row.mlRiskLevel === "high" || row.mlRiskLevel === "critical").forEach((row) => {
            notify({
              role: "admin", type: "SHORTAGE", severity: row.mlRiskLevel,
              ...
              relatedDrug: row.drugName, relatedBatch: row.batch, actionPath: "/admin/shortage-risk",
        ...
        if (path === "/api/admin/recalls" && req.method === "POST" && body?.id) {
          notify({
            role: null, type: "RECALL", severity: body.severity || "high",
            title: `Drug recall issued: ${body.drugName}`,
            message: `Batch ${body.batch} was recalled. Reason: ${body.reason}`,
            relatedDrug: body.drugName, relatedBatch: body.batch, actionPath: "/admin/recalls",
        ...
        if (/^\/api\/vendor\/stock-requests\/\d+\/approve$/.test(path) && req.method === "POST" && body?.request?.id) {
          notify({
            role: "distributor", type: "SHIPMENT",
            ...
            relatedDrug: body.request.drugName, relatedRequestId: body.request.id,
            actionPath: "/distributor/requests", eventKey: `dispatch:${body.request.id}:${body.request.dispatchedAt}`,
          });
        }

        if (/^\/api\/distributor\/stock-requests\/\d+\/receive$/.test(path) && req.method === "POST" && body?.request?.id) {
          notify({
            role: "vendor", type: "SHIPMENT", severity: body.mismatch?.mismatch ? "high" : "info",
            ...
            relatedDrug: body.request.drugName, relatedRequestId: body.request.id,
            actionPath: "/vendor/requests", eventKey: `receive:${body.request.id}:${body.request.receivedAt}`,
          });
        }

        if (/^\/api\/(vendor|distributor)\/.*cold-chain.*reading$/.test(path) && req.method === "POST" && body?.breached) {
          ...
          notify({
            role: "admin", type: "COLD_CHAIN", severity: "high",
            title: `Cold-chain breach: ${drug}`,
            message: (body.alerts || []).join("; ") || `Cold-chain limits were breached for batch ${batch}.`,
            relatedDrug: drug, relatedBatch: batch, actionPath: "/admin/anomalies",
...
app.get("/api/events", (req, res) => {
  ...
  req.on("close", () => { eventBus.off("update", ping); clearInterval(heartbeat); });
});

// ---------- Level 1: real Isolation Forest anomaly scan ----------
app.post("/api/admin/anomalies/scan", verifyToken, requireRole("admin"), (req, res) => {
  const db = getDB();
  const inventories = [["vendor", db.vendorInventory || []], ["distributor", db.distributorInventory || []], ["client", db.clientInventory || []]];
  const allItems = inventories.flatMap(([portal, items]) => items.map((item) => ({ portal, item })));
  const salesHistory = db.sales || [];
  if (allItems.length < 3) return res.status(400).json({ error: "At least 3 inventory records are required for Isolation Forest scanning." });
  ...
  results.forEach((result, index) => {
    ...
    const alreadyOpen = db.anomalies.some((a) => a.batch === item.batch && a.type === type && (a.status === "open" || a.status === "investigating"));
    if (alreadyOpen) return;
    const anomaly = { id: nextId("anomalies"), type, drugName: item.drugName, batch: item.batch, severity: result.severity, detectedAt: new Date().toISOString(), status: "open", source: portal, message: result.reason, model: "Isolation Forest", anomalyScore: result.score, features: result.features };
    db.anomalies.push(anomaly); created.push(anomaly);
    getChain().addBlock("ANOMALY_DETECTED", "system:isolation-forest", { portal, drugName: item.drugName, batch: item.batch, severity: result.severity, anomalyScore: result.score, model: "Isolation Forest" });
    createNotification({ role: "admin", type: "ANOMALY", severity: result.severity, title: `ML anomaly detected: ${item.drugName}`, message: result.reason, relatedDrug: item.drugName, relatedBatch: item.batch, actionPath: "/admin/anomalies", eventKey: `anomaly:${item.batch}:${Math.round(Number(result.score) * 1000)}` });
  });
  if (created.length) save();
  res.json({ model: "Isolation Forest", scannedRecords: allItems.length, anomaliesFound: results.filter((r) => r.anomaly).length, created, message: created.length ? `${created.length} new ML anomaly case(s) raised by Isolation Forest.` : "Isolation Forest found no new unusual inventory patterns." });
});
